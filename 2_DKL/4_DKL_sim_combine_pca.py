...
class GeneFeatureExtractor(torch.nn.Sequential):
    def __init__(self, dropout_rate=0.2):
        ...
        # Final linear layer
        self.add_module('linear3', torch.nn.Linear(50, 100))
...
class CombinedFeatureExtractor(torch.nn.Sequential):
    def __init__(self, dropout_rate=0.2):
        ...
        # First linear layer
        self.add_module('linear1', torch.nn.Linear(100, 50))
        self.add_module('relu1', torch.nn.ReLU())
        self.add_module('norm1', torch.nn.BatchNorm1d(50))  # Normalization layer
        self.add_module('drop1', torch.nn.Dropout(dropout_rate))  # Dropout layer

        # Second linear layer
        self.add_module('linear2', torch.nn.Linear(50, 5))
        self.add_module('relu2', torch.nn.ReLU())
...
class GPRegressionModel(gpytorch.models.ExactGP):
        ...
        def forward(self, x):
            ...
            train_x_gene = x[:,0:data_dim_gene]
            train_x_drug = x[:,data_dim_gene:]
            gene_features = self.gene_extractor(train_x_gene)
            drug_features = self.drug_extractor(train_x_drug)
            # reweight
            # Define weights for each set of features
            weight_gene = 0.2
            weight_drug = 1
                # Weight the features
            weighted_gene_features = gene_features * weight_gene
            weighted_drug_features = drug_features * weight_drug
            # combined_features = weighted_gene_features + drug_features

            # combined_features = weighted_gene_features * weighted_drug_features
            combined_features = torch.cat((weighted_gene_features, weighted_gene_features), dim=1)
...
plt.legend()
plt.grid(True)
plt.savefig('prediction_combine_pca3.png')